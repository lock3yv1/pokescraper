...
function isValid(title) {
  ...
  if (t.includes("<img") || EXCLUDE.some(k => t.includes(k))) return false;
  return (t.includes("pokemon") || t.includes("pokémon")) && SETS.some(s => t.includes(s)) && PRODUCT_KEYWORDS.some(k => t.includes(k));
}
